...
//DAC_DHR12RD_Address  12位计数 2^12 = 4096   所以DA 数值范围：0 - 4096
...
static void DAC_Config(void)
{
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;   //模拟输入 ?为什么是模拟输入，因为当DA的时候必须设置成模拟输入
  ...
  DAC_InitStructure.DAC_OutputBuffer = DAC_OutputBuffer_Disable;	//不使用DAC输出缓冲（当有缓冲功能的时候管脚的驱动能力就增强了，这里我们只是产生正弦波，不用驱动什么，所以不使用缓冲）
...
{	
  ...
  DMA_InitStructure.DMA_BufferSize = 32;																	//缓存大小为32 字节	
