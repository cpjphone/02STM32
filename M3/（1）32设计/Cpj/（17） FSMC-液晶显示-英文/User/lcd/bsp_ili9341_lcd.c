...
void LCD_FSMC_Config(void)
{
    ...
    FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAM4;  //块1的第四部分
...
void LCD_REG_Config(void)
{
	...
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0x36); 	      	/*36H这个寄存器管理扫描方式的  ☆☆☆*/
	LCD_ILI9341_Parameter(0xC8);    /*竖屏  左上角到(起点)到右下角(终点)扫描方式*/
	DEBUG_DELAY();
	
	/* column address control set */
	LCD_ILI9341_CMD(0X2A);            //2AH:横坐标 
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x00);			//起点：0000
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0xEF);			//终点：00EF  240
	
	/* page address control set */
	DEBUG_DELAY();
	LCD_ILI9341_CMD(0X2B); 					//2BH:竖坐标  
	LCD_ILI9341_Parameter(0x00);
	LCD_ILI9341_Parameter(0x00);		//起点：0000
	LCD_ILI9341_Parameter(0x01);
	LCD_ILI9341_Parameter(0x3F);		//终点：013F  319
...
void LCD_Clear(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
  ...
  LCD_ILI9341_CMD(0X2B); 			     
	...
	for( i=0; i < width*height; i++ )   //像素的数目
	{
		LCD_WR_Data( color );
		//Delay(0x0FFf);
	}	
...
void LCD_DispChar(uint16_t x, uint16_t y, uint8_t ascii, uint16_t color)
{
	...
	for( page=0; page < STR_HEIGHT; page++ )
	{
		...
		for( column=0; column < STR_WIDTH; column++ ) //一行6个点，1 取颜色 0取背景颜色
		{
			if( temp & 0x01 )
			{
				LCD_WR_Data( color );
...
void LCD_DispStr(uint16_t x, uint16_t y, uint8_t *pstr, uint16_t color)
{
	while( *pstr != '\0' )
	{
		...
		x += STR_WIDTH;   			// x = x +  STR_WIDTH  每次 X向右移动6位 因为是竖屏显示 所以是X增加
		pstr++;
	}
}
...
void LCD_DisNum(uint16_t x, uint16_t y, uint32_t num, uint16_t color)
{
	...
	if( temp == 0 )
	{
		LCD_DispChar(x, y, '0', color);   //数字加单引号转换成ASCII码
		return;
	}
