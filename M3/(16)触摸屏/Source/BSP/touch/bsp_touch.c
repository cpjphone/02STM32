...
//    cc2 = (touch_para.Fn*1.0)/touch_para.Divider; 
    ...
    {
      cal_flag = 0x55;
      SPI_FLASH_SectorErase(0);//spi写之前必须先擦除
      SPI_FLASH_BufferWrite(&cal_flag, 0, 1);     //矫正的标志位写到地址“0”里面
      SPI_FLASH_BufferWrite((void*)cal_p, 1, sizeof(cal_p)); //矫正参数从第一个地址里面开始写
      for( k=0; k<6; k++ )
       printf("\r\n tx = %LF \r\n",cal_p[k]); 	//写进去后，打印出来看一下
    }
...
void Palette_draw_point(uint16_t x, uint16_t y)
{
  ...
  if( x<40 )
  {
    ...
    {/* 清屏 */
      ...
      #elif 1
      LCD_Clear(40, 0, 280, 240, BLACK);	//触摸屏底色修改
      #endif
      return;
    }
  }
