...
/**-------------------------------------------------------------------------------
  ...
  * @retval     null
--------------------------------------------------------------------------------*/
unsigned char Read_MMA8452_Status(int address)
{   
    i2c_Start();                          //起始信号
    i2c_SendByte(SlaveAddress);           //发送设备地址+写信号
    i2c_SendByte(address);                   //发送存储单元地址，从0x01开始	
    i2c_Start();                          //起始信号
    i2c_SendByte(SlaveAddress+1);         //发送设备地址+读信号

    Status_Data = i2c_ReadByte();          //BUF[0]存储0x32地址中的数据
...
//	
//   Single_Write_MMA8452(0x2A,0x10);   // CTRL_REG1
//	delay_ms(100);
//   Single_Write_MMA8452(0x2B,0x02);   // CTRL_REG2
//	delay_ms(100);

	// Single_Write_MMA8452(0x2C,0x05);		//CTRL_REG3  PP_OD位
//	delay_ms(100);
//	 Single_Write_MMA8452(0x2D,0x01);		//CTRL_REG4   INT_EN_DRDY:读取数据引起中断使能
// 	 delay_ms(100);
//	 Single_Write_MMA8452(0x2E,0x01);	 	//CTRL_REG5   INT_CFG_DRDY
//	 delay_ms(100);
////	
//	Single_Write_MMA8452(0x23,0x18);
//	delay_ms(100);
//	Single_Write_MMA8452(0x24,0x18);
//	delay_ms(100);
//	Single_Write_MMA8452(0x25,0x18);
//	delay_ms(100);
//	
////	Single_Write_MMA8452(0x2D,0x20);		//CTRL_REG4   INT_EN_DRDY:读取数据引起中断使能
//// 	 delay_ms(100);
////	 Single_Write_MMA8452(0x2E,0x01);	 	//CTRL_REG5   INT_CFG_DRDY
////	 delay_ms(100);
////	
//	
//	Single_Write_MMA8452(0x2A,0x01);   // CTRL_REG1
//	delay_ms(100);
//	Single_Write_MMA8452(0x2B,0x02);   // CTRL_REG2
//	delay_ms(100);
	 
	  Single_Write_MMA8452(0x2A,0x18); 			//进入待机，100HZ的ODR，最低去抖响应时间为100ms
	//  delay_ms(100);
	 
	 
	 	Single_Write_MMA8452(0x15,0xF8);		//x，y方向运动检测和锁存
	 // delay_ms(100);
	 
		Single_Write_MMA8452(0x17,0x30);  	//设置阀值>3g
	  //delay_ms(100);
	 
	 
	 Single_Write_MMA8452(0x18,0x0A);				//去抖计数器100hz，计时器100ms，以消除虚假读书
	//  delay_ms(100);
	 
	 	 
	 Single_Write_MMA8452(0x2D,0x04);			 //启动系统中的运动/自由落体中断功能（CTRL_REG4）
	//  delay_ms(100);
	 
	  Single_Write_MMA8452(0x2E,0x04);  	//将运动/自由落体中断功能配置到INT1的硬件引脚（CTRL_REG5）
	 // delay_ms(100);
	 
	 Single_Write_MMA8452(0x2A,0x01);   	//将设备切换到主动模式
